...
impl AmmConfig {
    ...
    pub fn load_amm(account: &AccountView) -> Result<Ref<'_, Self>, ProgramError> {
        ...
        Ok(Ref::map(account.try_borrow()?, |data| unsafe { Self::from_bytes_unchecked(data) }))
    }
...
impl AmmConfig {
    ...
    pub fn load_amm_mut(account: &mut AccountView) -> Result<RefMut<'_, Self>, ProgramError> {
        ...
        Ok(RefMut::map(account.try_borrow_mut()?, |data| unsafe { Self::from_bytes_unchecked_mut(data) }))
    }
    ...
    #[inline(always)]
    pub fn set_all(&mut self, creator: Address, admin: Address, id: u32, fee: u16, paused: u8, bump: [u8; 1]) {
...
impl Pool {
    ...
    #[inline(always)]
    pub fn set_all(&mut self, amm: Address, mint_a: Address, mint_b: Address, bump: [u8; 1], mint_lp_bump: [u8; 1]) {
        self.amm = amm;
        self.mint_a = mint_a;
        self.mint_b = mint_b;
        self.bump = bump;
