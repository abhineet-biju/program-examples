...
pub fn process_instruction(_program_id: &Address, accounts: &mut [AccountView], data: &[u8]) -> ProgramResult {
    match data.split_first() {
        Some((InitializeAmm::DISCRIMINATOR, data)) => InitializeAmm::try_from((accounts, data))?.process(),
        Some((InitializePool::DISCRIMINATOR, data)) => InitializePool::try_from((accounts, data))?.process(),
        Some((DepositLiquidity::DISCRIMINATOR, data)) => DepositLiquidity::try_from((accounts, data))?.process(),
        Some((WithdrawLiquidity::DISCRIMINATOR, data)) => WithdrawLiquidity::try_from((accounts, data))?.process(),
        ...
        Some((TransferAdmin::DISCRIMINATOR, data)) => TransferAdmin::try_from((accounts, data))?.process(),
