...
impl<'a> TryFrom<&'a mut [AccountView]> for SwapExactTokensForTokensAccounts<'a> {
    ...
    fn try_from(accounts: &'a mut [AccountView]) -> Result<Self, Self::Error> {
        let [amm, pool_config, trader, mint_a, mint_b, pool_ata_a, pool_ata_b, trader_ata_a, trader_ata_b, payer, token_program, associated_token_program, system_program] =
            accounts
        else {
...
impl TryFrom<&[u8]> for SwapExactTokensForTokensInstructionData {
    ...
    fn try_from(data: &[u8]) -> Result<Self, Self::Error> {
        ...
        let input_amount = u64::from_le_bytes(data[1..9].try_into().map_err(|_| ProgramError::InvalidInstructionData)?);

        let min_output_amount =
            u64::from_le_bytes(data[9..].try_into().map_err(|_| ProgramError::InvalidInstructionData)?);

        Ok(Self { swap_a, input_amount, min_output_amount })
    }
}
...
impl<'a> SwapExactTokensForTokens<'a> {
    ...
    pub fn process(&mut self) -> ProgramResult {
        ...
        let (trader_balance_a, trader_balance_b, pool_balance_a, pool_balance_b) = {
            ...
            (trader_ata_a.amount(), trader_ata_b.amount(), pool_ata_a.amount(), pool_ata_b.amount())
        };
        ...
        if self.instruction_data.swap_a {
            Transfer::new(self.accounts.trader_ata_a, self.accounts.pool_ata_a, self.accounts.trader, input)
                .invoke()?;

            Transfer::new(self.accounts.pool_ata_b, self.accounts.trader_ata_b, self.accounts.pool_config, output)
                .invoke_signed(&[pool_signer])?;
        } else {
            Transfer::new(self.accounts.pool_ata_a, self.accounts.trader_ata_a, self.accounts.pool_config, output)
                .invoke_signed(&[pool_signer])?;

            Transfer::new(self.accounts.trader_ata_b, self.accounts.pool_ata_b, self.accounts.trader, input)
                .invoke()?;
        }
