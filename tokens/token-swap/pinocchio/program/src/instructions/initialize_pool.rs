...
impl<'a> TryFrom<&'a mut [AccountView]> for InitializePoolAccounts<'a> {
    ...
    fn try_from(accounts: &'a mut [AccountView]) -> Result<Self, Self::Error> {
        let [payer, amm, pool_config, mint_lp, mint_a, mint_b, pool_ata_a, pool_ata_b, token_program, associated_token_program, system_program] =
            accounts
        else {
        ...
        let (expected_pool_ata_a, _) = Address::derive_program_address(
            &[pool_config.address().as_ref(), pinocchio_token::ID.as_ref(), mint_a.address().as_ref()],
            &pinocchio_associated_token_account::ID,
        )
        .ok_or(ProgramError::InvalidSeeds)?;

        let (expected_pool_ata_b, _) = Address::derive_program_address(
            &[pool_config.address().as_ref(), pinocchio_token::ID.as_ref(), mint_b.address().as_ref()],
            &pinocchio_associated_token_account::ID,
        )
        .ok_or(ProgramError::InvalidSeeds)?;
...
impl<'a> InitializePool<'a> {
    ...
    pub fn process(&mut self) -> ProgramResult {
        ...
        InitializeMint2::new(self.accounts.mint_lp, 6, self.accounts.pool_config.address(), None).invoke()?;
