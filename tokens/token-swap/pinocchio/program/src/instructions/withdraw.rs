...
use crate::{Pool, ID, MINIMUM_LIQUIDITY};
...
impl<'a> TryFrom<&'a mut [AccountView]> for WithdrawLiquidityAccounts<'a> {
    ...
    fn try_from(accounts: &'a mut [AccountView]) -> Result<Self, Self::Error> {
        let [pool_config, depositor, mint_lp, mint_a, mint_b, pool_ata_a, pool_ata_b, depositor_ata_lp, depositor_ata_a, depositor_ata_b, payer, token_program, associated_token_program, system_program] =
            accounts
        else {
...
impl<'a> WithdrawLiquidity<'a> {
    ...
    pub fn process(&mut self) -> ProgramResult {
        ...
        let amount_a = ((self.instruction_data.amount as u128) * (pool_balance_a as u128) / total_liquidity) as u64;
        let amount_b = ((self.instruction_data.amount as u128) * (pool_balance_b as u128) / total_liquidity) as u64;
        ...
        Transfer::new(self.accounts.pool_ata_a, self.accounts.depositor_ata_a, self.accounts.pool_config, amount_a)
            .invoke_signed(&pool_signers)?;

        Transfer::new(self.accounts.pool_ata_b, self.accounts.depositor_ata_b, self.accounts.pool_config, amount_b)
            .invoke_signed(&pool_signers)?;
