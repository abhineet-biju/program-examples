...
impl<'a> TryFrom<&'a mut [AccountView]> for InitializeAmmAccounts<'a> {
    ...
    fn try_from(accounts: &'a mut [AccountView]) -> Result<Self, Self::Error> {
        ...
        Ok(Self { payer, admin, amm, system_program })
    }
}
...
impl<'a> TryFrom<&'a [u8]> for InitializeAmmInstructionData {
    ...
    fn try_from(data: &'a [u8]) -> Result<Self, Self::Error> {
        ...
        let id = u32::from_le_bytes(data[0..4].try_into().map_err(|_| ProgramError::InvalidInstructionData)?);

        let fee = u16::from_le_bytes(data[4..6].try_into().map_err(|_| ProgramError::InvalidInstructionData)?);
...
impl<'a> InitializeAmm<'a> {
    ...
    pub fn process(&mut self) -> ProgramResult {
        ...
        let (expected_amm, canonical_bump) =
            Address::derive_program_address(&[b"amm", self.accounts.admin.address().as_array(), &amm_id], &ID)
