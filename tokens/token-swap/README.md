...
This example implements a constant-product automated market maker for two SPL tokens.
...
The Pinocchio implementation mints later LP tokens according to their share of the existing reserves and LP supply.
...
## Run the Pinocchio example

```bash
cd tokens/token-swap/pinocchio
pnpm install
pnpm build-and-test
```

The tests use LiteSVM and `@solana/kit`.

## Scope

This is an educational example. It does not include deposit or withdrawal slippage limits, oracle pricing, protocol fees, or Token-2022 extensions.
